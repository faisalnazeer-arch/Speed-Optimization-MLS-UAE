  ...
  class CartItems extends HTMLElement {
    ...
    updateShippingBar() {
      fetch('/cart.js')
        ...
        .then(cart => {
          ...
          // ✅ Detect subscription products + calculate subscription total
          ...
          const hasValidSubscription = subscriptionTotal >= 100;
          ...
          if (this.milestoneBarFill && this.milestoneMessage) {
            for (let i = 0; i < this.milestones.length; i++) {
              // ✅ Force first milestone active ONLY if subscription ≥ 100
              if ((i === 0 && hasValidSubscription) || total >= this.milestones[i]) {
            ...
            // ✅ If subscription ≥ 100, pretend milestone 1 is met
            let adjustedTotal = total;
            if (hasValidSubscription && this.milestones.length > 0) {
              adjustedTotal = Math.max(total, this.milestones[0]);
            }

            this.milestoneBarFill.style.width = `${Math.min((adjustedTotal / this.milestones[this.milestones.length - 1]) * 100, 100)}%`;

            const nextMilestone = this.milestones.find(m => adjustedTotal < m);
            if (nextMilestone) {
    ...
    async updateQuantity(line, quantity, name) {
      ...
      try {
        ...
        this.getSectionsToRender().forEach((section) => {
          ...
          // Update the cart drawer quantity
          if (section.id === 'cart-icon-bubble') {
            const drawerCount = document.getElementById('cart-drawer__count');
            if (drawerCount) {
              const html = CartItems.getElementHTML(
                data.sections[section.section],
                `${section.selector} .header__cart-count-number`
              );

              drawerCount.innerHTML = `(${html})`;
            }
          }
        });
        ...
        // Reinitialize map after quantity update
        if (window.safeInitCartMap) {
          setTimeout(() => {
            console.log("🔄 Quantity updated, reinitializing map");
            window.safeInitCartMap();
          }, 500);
        }
      }
    }
    ...
    async refreshCartItems() {
      ...
      try {
        ...
        if (response.status === 400 && this.cartDrawer) {
          this.cartDrawer.refreshCartDrawer(true);
          return;
        }
        ...
        // Reinitialize map after cart refresh
        if (window.safeInitCartMap) {
          setTimeout(() => {
            console.log("🔄 Cart refreshed, reinitializing map");
            window.safeInitCartMap();
          }, 500);
        }
      } catch (error) {
    ...
    getSectionsToRender() {
      ...
      if (this.cartDrawer) {
        ...
        // Append any quick order lists to the sections
