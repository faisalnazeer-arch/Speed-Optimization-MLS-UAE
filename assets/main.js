...
class StoreHeader extends HTMLElement {
  ...
  bindEvents() {
    ...
    if (Shopify.designMode) {
      document.addEventListener('shopify:section:load', this.resizeHandler);
    }
  }
  ...
  setMenuHeight() {
    if (this.menu && this.menu.clientHeight) {
      this.style.setProperty('--menu-height', `${this.menu.getBoundingClientRect().height.toFixed(2)}px`);
      document.documentElement.style.setProperty('--header-height', `${this.getBoundingClientRect().height.toFixed(2)}px`);
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    if (window.setDimensionVariables) window.setDimensionVariables();
  }, 0);
...
window.onload = function() {
  document.querySelectorAll('.pickup-status .mb-0 strong').forEach(function(strongTag) {
    var brTag = document.createElement('br');
    strongTag.parentNode.insertBefore(brTag, strongTag);
  });
};