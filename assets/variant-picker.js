...
if (!customElements.get('variant-picker')) {
  class VariantPicker extends HTMLElement {
    ...
    static updateOptionAvailability(optionEl, exists, available) {
      ...
      if (optionEl.classList.contains('custom-select__option')) {
        ...
        if (!available) {
          ...
          const visibleLabel = document.querySelector('label.second_option:not(.hide)');

          if (visibleLabel) {
            visibleLabel.click();
            console.log(`Auto-clicked label for input: ${visibleLabel.getAttribute('for')}`);
          } else {
            console.warn('No visible label with class .second_option found');
          }
        ...
        el.classList.add('hide');
        if (el.nextElementSibling) el.nextElementSibling.title = unavailableText;
      } else {
        el.classList.remove('hide');
        if (el.nextElementSibling) el.nextElementSibling.removeAttribute('title');
      }
    }
    ...
    /**
     * Updates the per-kg price display based on the selected variant metafield.
     * Falls back to window.productKgPriceFallback (product-level) if the variant
     * has no price_per_kg set.
     */
    updatePerKgPrice() {
      this.perKgPriceEl = this.perKgPriceEl || this.section.querySelector('#per-kg-price-display');
      if (!this.perKgPriceEl) return;
      ...
      if (this.variant) {
        ...
        const metafields =
          ...
          window.productMetafields.metafields &&
          window.productMetafields.metafields[this.variant.id];

        const variantValue = metafields && metafields.price_per_kg
          ? metafields.price_per_kg.trim()
          : '';

        // Fall back to product-level if variant has no value
        value = variantValue !== ''
          ? variantValue
          : (window.productKgPriceFallback ? window.productKgPriceFallback.trim() : '');
      }

      const strongEl = this.perKgPriceEl.querySelector('strong');
      if (value !== '') {
        if (strongEl) strongEl.textContent = value;
        this.perKgPriceEl.style.display = 'block';
      } else {
        this.perKgPriceEl.style.display = 'none';
      }
    }
...
function updateSecondOptionBasedOnFirst() {
  const firstChecked = document.querySelector('input.first_option:checked');
  if (!firstChecked) return;

  const visibleLabel = document.querySelector('label.second_option:not(.hide)');
  const input = document.querySelector('input.second_option:not(.hide)');
  console.log("abc");
  if (visibleLabel && input) {
    console.log("ggg");
    input.checked = true;
    input.dispatchEvent(new Event('change', { bubbles: true }));
  }
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('input.first_option').forEach(function (el) {
    el.addEventListener('change', function () {
      updateSecondOptionBasedOnFirst();
    });
  });
});